...
async function updateContractData() {
    try {
        // Убедимся, что сеть подгружена
        await provider.getNetwork();

        // Проверяем код напрямую через провайдер
        const code = await provider.getCode(CONTRACT_ADDRESS);
        console.log("Contract code length:", code.length);

        if (code === "0x" || code === "0x0") {
            console.error("Контракт всё еще не виден. Попробуй 'Сброс аккаунта' в MetaMask еще раз.");
            return;
        }

        const unlockTime = await contract.unlockTime();
        unlockTimeSpan.innerText = new Date(Number(unlockTime) * 1000).toLocaleString();
